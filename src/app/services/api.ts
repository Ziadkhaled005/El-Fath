const API_BASE_URL = (import.meta.env.VITE_API_BASE_URL || 'https://medshareappapi.runasp.net').replace(/\/$/, '');
...
export const authApi = {
  async login(username: string, password: string) {
    const data = await request<{ accessToken?: string; refreshToken?: string; user?: unknown }>('/api/Auth/login', {
      method: 'POST',
      body: JSON.stringify({ username, password }),
    }, false);
    if (data.accessToken) setStoredTokens(data.accessToken, data.refreshToken);
    return data;
  },
...
export const productsApi = {
  ...
  async update(id: number, payload: Record<string, unknown>) {
    return request(`/api/Products/${id}`, { method: 'PUT', body: JSON.stringify({ id, ...payload }) });
  },
  ...
  async adjustStock(id: number, delta: number, reason?: string) {
    ...
    return request(`/api/Products/${id}/adjust-stock?${params.toString()}`, { method: 'POST' });
  },
};
...
export const customersApi = {
  ...
  async update(id: number, payload: Record<string, unknown>) {
    return request(`/api/Customers/${id}`, { method: 'PUT', body: JSON.stringify({ id, ...payload }) });
  },
...
export function normalizeCollection<T>(payload: unknown): T[] {
  ...
  if (payload && typeof payload === 'object') {
    const container = payload as { items?: unknown; data?: unknown; result?: unknown; value?: unknown };
