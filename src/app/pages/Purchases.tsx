...
export function Purchases() {
    ...
    const [viewItem, setViewItem] = useState<(typeof PURCHASES)[0] | null>(
        null,
    );
    ...
    useEffect(() => {
        const loadPurchases = async () => {
            try {
                const payload = await purchasesApi.list({
                    page: 1,
                    pageSize: 100,
                });
                const items =
                    normalizeCollection<(typeof PURCHASES)[0]>(payload);
                if (items.length > 0) {
                    setPurchases(items);
                }
            } catch {
    ...
    const addPurchase = async () => {
        if (!newForm.supplier || !newForm.total) {
        ...
        try {
            ...
            addToast({
                type: "error",
                message: "تعذر الاتصال بالخادم، تم إضافة الطلب محلياً فقط",
            });
        }
        setShowNew(false);
                    ...
                    <table
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                filtered.map((p) => {
                                    const s =
                                        STATUS[p.status as keyof typeof STATUS];
                                    return (
                                        <tr
                                            key={p.id}
                                            style={{
