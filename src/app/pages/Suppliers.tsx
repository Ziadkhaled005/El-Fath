...
import { normalizeCollection, suppliersApi } from "../services/api";
...
export function Suppliers() {
    ...
    useEffect(() => {
        const loadSuppliers = async () => {
            try {
                const payload = await suppliersApi.list({
                    page: 1,
                    pageSize: 100,
                });
                const items = normalizeCollection<Supplier>(payload);
                if (items.length > 0) {
                    setSuppliers(items);
                }
            } catch {
    ...
    const save = async () => {
        if (!form.name) {
            addToast({ type: "error", message: "الاسم مطلوب" });
            return;
        }
        if (editItem) {
            setSuppliers((prev) =>
            ...
            try {
                ...
                addToast({
                    type: "error",
                    message: "تعذر الاتصال بالخادم، تم الإضافة محلياً فقط",
                });
            }
        }
        setShowForm(false);
            ...
            <div style={{ padding: 24 }}>
                ...
                <div
                    ...
                    {[
                        ...
                        {
                            label: "أرصدة مستحقة",
                            value: suppliers.filter((s) => s.balance > 0)
                                .length,
                            color: "#EF4444",
                        },
                    ].map((s, i) => (
                ...
                <div
                    ...
                    <div
                        ...
                        {[
                            ...
                            [
                                "رصيد مستحق",
                                viewItem.balance.toLocaleString("ar-EG") +
                                    " ج.م",
                            ],
                            [
                                "إجمالي المشتريات",
